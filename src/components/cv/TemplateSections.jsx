...
export function PhotoSection({ data, config, styles }) {
  ...
  return (
    <div className="flex flex-col items-center">
      ...
      {showName && full_name && (
        <h2 className={`mt-3 font-bold ${preview ? 'text-sm' : 'text-xl'}`}>{full_name}</h2>
      )}
      {showTitle && target_position && (
        <p className={`text-gray-600 ${preview ? 'text-xs' : 'text-sm'}`}>{target_position}</p>
      )}
    </div>
  );
}
...
export function HeaderSection({ data, config, styles }) {
  const { full_name, target_position, email, phone, location, linkedin_url } = data;
  ...
  if (layout === 'centered') {
    return (
      <div className={`text-center ${border ? 'border-b-2 border-gray-800 pb-4 mb-6' : 'mb-6'}`}>
        <h1 className={`font-bold ${preview ? 'text-lg' : 'text-3xl'} mb-2`}>{full_name || 'Your Name'}</h1>
        {target_position && <p className={`text-gray-600 mb-2 ${preview ? 'text-xs' : 'text-base'}`}>{target_position}</p>}
  ...
  if (layout === 'horizontal') {
    return (
      <div className={`flex gap-4 ${border ? 'border-b-2 border-gray-800 pb-4 mb-6' : 'mb-6'}`}>
        {showPhoto && <PhotoSection data={data} config={{ size: 'small' }} styles={styles} />}
        <div className="flex-1">
          <h1 className={`font-bold ${preview ? 'text-base' : 'text-2xl'} mb-1`}>{full_name || 'Your Name'}</h1>
          <p className={`text-gray-600 mb-2 ${preview ? 'text-xs' : 'text-sm'}`}>
            {contactInfo.join(' | ')}
          </p>
        </div>
      </div>
  ...
  if (layout === 'dark') {
    return (
      <div className="bg-slate-700 text-white p-6 flex items-center gap-6 mb-0">
        {showPhoto && <PhotoSection data={data} config={{ circular: true, size: 'medium' }} styles={styles} />}
        <div>
          <h1 className={`font-bold uppercase tracking-wide ${preview ? 'text-lg' : 'text-3xl'}`}>{full_name || 'Your Name'}</h1>
          <p className={`mt-1 ${preview ? 'text-xs' : 'text-base'}`}>{target_position || ''}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-2">
      {items.map(item => (
        <div key={item.label} className={`${preview ? 'text-xs' : 'text-sm'}`}>
          {showLabels && <span className="font-semibold block">{item.label}</span>}
...
export function SummarySection({ data, config, styles }) {
  ...
  return (
    <section className="mb-6">
      <h2 className={`font-bold ${border ? 'border-b border-gray-800 pb-2 mb-3' : 'mb-3'} ${preview ? 'text-xs' : 'text-base'}`}>
        {title}
      </h2>
      <p className={`text-gray-700 leading-relaxed ${preview ? 'text-[10px]' : 'text-sm'}`}>{summary}</p>
    </section>
  );
}
...
export function ExperienceSection({ data, config, styles }) {
  ...
  const validExperiences = experiences.filter(e => e.job_title || e.company).slice(0, maxItems);
  if (validExperiences.length === 0) return null;
  
  return (
    <section className="mb-6">
      <h2 className={`font-bold border-b border-gray-800 pb-2 mb-4 ${preview ? 'text-xs' : 'text-base'}`}>
        {title}
      </h2>
          ...
          <div key={idx}>
            {layout === 'timeline' ? (
              <div className="relative pl-4 border-l-2 border-gray-300 pb-4">
                ...
                <div className={`font-bold ${preview ? 'text-xs' : 'text-sm'}`}>{exp.job_title}</div>
                {showCompany && <div className={`text-gray-600 ${preview ? 'text-[10px]' : 'text-xs'}`}>{exp.company}</div>}
                <div className={`text-gray-500 ${preview ? 'text-[10px]' : 'text-xs'}`}>
                  {formatDate(exp.start_date, exp.end_date)}
                </div>
                {exp.achievements && (
                  <ul className={`list-disc list-outside ml-4 mt-1 text-gray-700 ${preview ? 'text-[10px]' : 'text-xs'}`}>
              ...
              <>
                ...
                {(showCompany || showLocation) && (
                  <div className={`text-gray-600 mb-1 ${preview ? 'text-[10px]' : 'text-xs'}`}>
                    {[exp.company, exp.location].filter(Boolean).join(', ')}
                  </div>
                )}
                {exp.achievements && (
                  <ul className={`list-disc list-outside ml-5 space-y-1 text-gray-700 ${preview ? 'text-[10px]' : 'text-xs'}`}>
                    {exp.achievements.split('\n').filter(a => a.trim()).map((a, i) => (
                      <li key={i}>{a.trim()}</li>
                    ))}
                  </ul>
...
export function EducationSection({ data, config, styles }) {
  ...
  const validEducation = education.filter(e => e.degree || e.university).slice(0, maxItems);
    ...
    <section className="mb-6">
      ...
      <div className="space-y-3">
        {validEducation.map((edu, idx) => (
          <div key={idx} className={layout === 'compact' ? 'flex justify-between items-center' : ''}>
            <div>
              <div className={`font-bold ${preview ? 'text-xs' : 'text-sm'}`}>{edu.degree}</div>
              <div className={`text-gray-600 ${preview ? 'text-[10px]' : 'text-xs'}`}>
                {edu.university}{showLocation && edu.location && `, ${edu.location}`}
              </div>
            </div>
            <div className={`text-gray-500 ${preview ? 'text-[10px]' : 'text-xs'}`}>
...
export function SkillsSection({ data, config, styles }) {
  ...
  const skillList = skills.split(',').map(s => s.trim()).filter(Boolean).slice(0, maxItems);
  if (skillList.length === 0) return null;
  
  return (
    <section className="mb-6">
      <h2 className={`font-bold border-b border-gray-800 pb-2 mb-3 ${preview ? 'text-xs' : 'text-base'}`}>
        {title}
      </h2>
      {layout === 'table' ? (
        <div className="grid grid-cols-2 gap-x-8 gap-y-1">
          {skillList.map((skill, idx) => (
            <div key={idx} className="grid grid-cols-2 gap-2">
              <span className={`text-gray-900 ${preview ? 'text-[10px]' : 'text-xs'}`}>{skill}</span>
              {showLevel && <span className={`text-gray-700 ${preview ? 'text-[10px]' : 'text-xs'}`}>Expert</span>}
            </div>
          ))}
        </div>
      ) : layout === 'tags' ? (
        <div className="flex flex-wrap gap-2">
          {skillList.map((skill, idx) => (
            <span key={idx} className={`bg-gray-100 px-2 py-1 rounded text-gray-800 ${preview ? 'text-[10px]' : 'text-xs'}`}>
              {skill}
            </span>
          ))}
        </div>
      ) : (
        <p className={`text-gray-700 ${preview ? 'text-[10px]' : 'text-sm'}`}>{skillList.join(', ')}</p>
      )}
    </section>
  );
}
    ...
    <section className="mb-6">
      ...
      <div className="space-y-1">
        {langList.map((lang, idx) => (
          <div key={idx} className={`text-gray-700 ${preview ? 'text-[10px]' : 'text-sm'}`}>• {lang}</div>
        ))}
      </div>
    </section>
  );
